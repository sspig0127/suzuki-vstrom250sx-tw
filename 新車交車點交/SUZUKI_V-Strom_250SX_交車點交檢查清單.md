...
- [ ] **儀表警示燈逐一指認意義**(現場請業務對照儀表實際亮燈位置說明,音檔文字描述的左右配置建議現場再次確認,參考時間軸 [06:47](https://youtu.be/Wcv19vGNH6g?t=407)~[11:25](https://youtu.be/Wcv19vGNH6g?t=685)):
  ...
  - [ ] **切換儀表到「電壓表」畫面**（下一項設定發動機轉速指示燈的前置動作）：多功能顯示區的內容分兩組切換——按 **SEL** 鍵在「里程表 → 行程表 A → 行程表 B」間切換；按 **ADJ** 鍵在「平均油耗表 A → 平均油耗表 B → 瞬時油耗表 → 電壓表」間切換。持續短按 ADJ 鍵，直到畫面出現電壓表（顯示電池電壓，正常範圍 10.0～16.0V）為止（中/英文版 P.2-25~2-26、日文版 P.55）。
